...
def insert_stock(db: Session, stock_data: dict):
    ...
    try:
        print(f"🆕 Inserting new stock record for {stock_data['symbol']} at {stock_data['timestamp']}")
        stock = Stock(**stock_data)  # No need to set `id`, PostgreSQL will auto-generate it
        db.add(stock)
        db.commit()
